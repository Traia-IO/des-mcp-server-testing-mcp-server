...
# Script to build and run the Des MCP Server Testing MCP Server locally in Docker
...
IMAGE_NAME="des-mcp-server-testing-mcp-server"
CONTAINER_NAME="des-mcp-server-testing-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Des MCP Server Testing MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Des MCP Server Testing MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Des MCP Server Testing MCP Server is running!${NC}"
echo
