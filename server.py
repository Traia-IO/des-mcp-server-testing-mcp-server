#!/usr/bin/env python3
"""
Des MCP Server Testing MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('des-mcp-server-testing_mcp')
...
logger.info("="*80)
logger.info(f"Des MCP Server Testing MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://petstore3.swagger.io/api/v3")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Des MCP Server Testing MCP Server", host="0.0.0.0")
...
# - DES_MCP_SERVER_TESTING_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="des-mcp-server-testing-mcp-server"  # MCP server ID for tracking
    )
    ...
    async def health_check(request: Request) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                "status": "healthy",
                "service": "des-mcp-server-testing-mcp-server",
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Des MCP Server Testing MCP Server")
    logger.info("="*80)
